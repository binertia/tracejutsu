//go:build linux && (amd64 || arm64)

package ebpf

import (
	"bytes"
	"context"
	"encoding/binary"
	"errors"
	"fmt"
	"os"
	"path/filepath"
	"sync/atomic"
	"time"

	cebpf "github.com/cilium/ebpf"
	"github.com/cilium/ebpf/asm"
	"github.com/cilium/ebpf/link"
	"github.com/cilium/ebpf/ringbuf"
	"github.com/cilium/ebpf/rlimit"

	"tracejutsu/internal/events"
)

type NetworkServerCollector struct {
	host           string
	procRoot       string
	containerCache *containerMetadataCache
	ringBufferSize int
	metrics        collectorMetrics
	sequence       atomic.Uint64
}

type networkServerRecord struct {
	KernelTimestampNS           uint64
	CompletionKernelTimestampNS uint64
	ReturnValue                 int64
	PID                         uint32
	UID                         uint32
	Comm                        [commSize]byte
	Sockaddr                    [sockaddrStorageSize]byte
	FD                          int32
	Backlog                     int32
	Syscall                     uint32
	_                           uint32
}

func NewNetworkServerCollector() (*NetworkServerCollector, error) {
	return NewNetworkServerCollectorWithConfig(RuntimeConfig{})
}

func NewNetworkServerCollectorWithConfig(config RuntimeConfig) (*NetworkServerCollector, error) {
	config, err := checkedRuntimeConfig(config)
	if err != nil {
		return nil, err
	}
	host, err := os.Hostname()
	if err != nil {
		return nil, fmt.Errorf("read hostname: %w", err)
	}
	return &NetworkServerCollector{
		host:           host,
		procRoot:       "/proc",
		containerCache: newContainerMetadataCache(),
		ringBufferSize: config.RingBufferSize,
	}, nil
}

func (collector *NetworkServerCollector) Run(ctx context.Context, sink chan<- events.Event) error {
	if sink == nil {
		return errors.New("event sink is required")
	}
	_ = rlimit.RemoveMemlock()

	records, err := cebpf.NewMap(&cebpf.MapSpec{Type: cebpf.RingBuf, Name: "rg_netsrv_rb", MaxEntries: uint32(collector.ringBufferSize)})
	if err != nil {
		return fmt.Errorf("create network server ring buffer: %w", err)
	}
	defer records.Close()

	pending, err := newPendingSyscallMap("rg_netsrv_pend", binary.Size(networkServerRecord{}))
	if err != nil {
		return fmt.Errorf("create pending network server map: %w", err)
	}
	defer pending.Close()

	drops, err := newDropCounterMap("rg_netsrv_drop")
	if err != nil {
		return fmt.Errorf("create network server drop counter: %w", err)
	}
	defer drops.Close()
	collector.metrics.attachDropCounter(drops)
	defer collector.metrics.detachDropCounter(drops)

	correlationDrops, err := newDropCounterMap("rg_netsrv_cd")
	if err != nil {
		return fmt.Errorf("create network server correlation drop counter: %w", err)
	}
	defer correlationDrops.Close()
	collector.metrics.attachCorrelationDropCounter(correlationDrops)
	defer collector.metrics.detachCorrelationDropCounter(correlationDrops)

	enterProgram, err := cebpf.NewProgram(networkServerEnterProgramSpec(pending.FD(), correlationDrops.FD()))
	if err != nil {
		return fmt.Errorf("load network server sys_enter raw tracepoint program: %w", err)
	}
	defer enterProgram.Close()

	enterHook, err := link.AttachRawTracepoint(link.RawTracepointOptions{Name: "sys_enter", Program: enterProgram})
	if err != nil {
		return fmt.Errorf("attach sys_enter raw tracepoint: %w", err)
	}
	defer enterHook.Close()

	exitProgram, err := cebpf.NewProgram(networkServerExitProgramSpec(records.FD(), drops.FD(), pending.FD()))
	if err != nil {
		return fmt.Errorf("load network server sys_exit raw tracepoint program: %w", err)
	}
	defer exitProgram.Close()

	exitHook, err := link.AttachRawTracepoint(link.RawTracepointOptions{Name: "sys_exit", Program: exitProgram})
	if err != nil {
		return fmt.Errorf("attach sys_exit raw tracepoint: %w", err)
	}
	defer exitHook.Close()

	reader, err := ringbuf.NewReader(records)
	if err != nil {
		return fmt.Errorf("open network server ring buffer reader: %w", err)
	}
	defer reader.Close()

	stopReaderClose := closeOnContextCancel(ctx, reader)
	defer stopReaderClose()

	for {
		record, err := reader.Read()
		if err != nil {
			if errors.Is(err, ringbuf.ErrClosed) && ctx.Err() != nil {
				return nil
			}
			return fmt.Errorf("read network server ring buffer: %w", err)
		}
		raw, err := decodeNetworkServerRecord(record.RawSample)
		if err != nil {
			return err
		}
		event, ok := collector.normalize(raw)
		if !ok {
			continue
		}
		select {
		case sink <- event:
		case <-ctx.Done():
			return nil
		}
	}
}

func (collector *NetworkServerCollector) Stats() Stats {
	return collector.metrics.stats()
}

func (*NetworkServerCollector) Name() string {
	return CollectorNetworkServer
}

func networkServerEnterProgramSpec(pendingFD, correlationDropCounterFD int) *cebpf.ProgramSpec {
	const (
		pidOffset      = int16(24)
		uidOffset      = int16(28)
		commOffset     = int16(32)
		sockaddrOffset = int16(48)
		fdOffset       = int16(80)
		backlogOffset  = int16(84)
		syscallOffset  = int16(88)
	)
	recordSize := int16(binary.Size(networkServerRecord{}))
	recordStart := -recordSize
	keyOffset := recordStart - 8
	tempPointer := keyOffset - 8
	tempValue := tempPointer - 8

	instructions := asm.Instructions{
		asm.Mov.Reg(asm.R6, asm.R1),
		asm.LoadMem(asm.R8, asm.R6, 8, asm.DWord),
		asm.JEq.Imm(asm.R8, bindSyscallNumber, "capture_bind"),
		asm.JEq.Imm(asm.R8, listenSyscallNumber, "capture_listen"),
		asm.Ja.Label("exit"),
		asm.Mov.Imm(asm.R0, 0).WithSymbol("init"),
	}
	for offset := recordStart; offset < 0; offset += 8 {
		instructions = append(instructions, asm.StoreMem(asm.RFP, offset, asm.R0, asm.DWord))
	}
	instructions = append(instructions,
		asm.StoreMem(asm.RFP, recordStart+syscallOffset, asm.R8, asm.Word),
		asm.FnKtimeGetNs.Call(),
		asm.StoreMem(asm.RFP, recordStart, asm.R0, asm.DWord),
		asm.FnGetCurrentPidTgid.Call(),
		asm.StoreMem(asm.RFP, keyOffset, asm.R0, asm.DWord),
		asm.RSh.Imm(asm.R0, 32),
		asm.StoreMem(asm.RFP, recordStart+pidOffset, asm.R0, asm.Word),
		asm.FnGetCurrentUidGid.Call(),
		asm.StoreMem(asm.RFP, recordStart+uidOffset, asm.R0, asm.Word),
		asm.Mov.Reg(asm.R1, asm.RFP),
		asm.Add.Imm(asm.R1, int32(recordStart+commOffset)),
		asm.Mov.Imm(asm.R2, commSize),
		asm.FnGetCurrentComm.Call(),
	)
	instructions = append(instructions, asm.Mov.Imm(asm.R0, 0).WithSymbol("capture_bind"))
	instructions = append(instructions,
		readRegisterArgument(asm.R6, syscallArg0Offset, asm.RFP, tempValue, recordStart+fdOffset, asm.Word)...,
	)
	instructions = append(instructions,
		asm.LoadMem(asm.R3, asm.R6, 0, asm.DWord),
		asm.Add.Imm(asm.R3, syscallArg1Offset),
		asm.Mov.Reg(asm.R1, asm.RFP),
		asm.Add.Imm(asm.R1, int32(tempPointer)),
		asm.Mov.Imm(asm.R2, 8),
		asm.FnProbeReadKernel.Call(),
		asm.LoadMem(asm.R3, asm.RFP, tempPointer, asm.DWord),
		asm.Mov.Reg(asm.R1, asm.RFP),
		asm.Add.Imm(asm.R1, int32(recordStart+sockaddrOffset)),
		asm.Mov.Imm(asm.R2, sockaddrStorageSize),
		asm.FnProbeReadUser.Call(),
		asm.Ja.Label("emit"),
	)
	instructions = append(instructions, asm.Mov.Imm(asm.R0, 0).WithSymbol("capture_listen"))
	instructions = append(instructions,
		readRegisterArgument(asm.R6, syscallArg0Offset, asm.RFP, tempValue, recordStart+fdOffset, asm.Word)...,
	)
	instructions = append(instructions,
		readRegisterArgument(asm.R6, syscallArg1Offset, asm.RFP, tempValue, recordStart+backlogOffset, asm.Word)...,
	)
	instructions = append(instructions, asm.Mov.Imm(asm.R0, 0).WithSymbol("emit"))
	instructions = append(instructions, storePendingSyscall(pendingFD, keyOffset, recordStart)...)
	instructions = append(instructions, countCorrelationDrop(correlationDropCounterFD, tempValue)...)
	instructions = append(instructions, asm.Mov.Imm(asm.R0, 0).WithSymbol("exit"), asm.Return())
	return &cebpf.ProgramSpec{Name: "rg_netsrv_enter", Type: cebpf.RawTracepoint, License: "GPL", Instructions: instructions}
}

func networkServerExitProgramSpec(ringBufferFD, dropCounterFD, pendingFD int) *cebpf.ProgramSpec {
	const (
		completionTimestampOffset = int16(8)
		returnValueOffset         = int16(16)
	)
	return completedSyscallProgramSpec("rg_netsrv_exit", ringBufferFD, dropCounterFD, pendingFD, int16(binary.Size(networkServerRecord{})), completionTimestampOffset, returnValueOffset)
}

func decodeNetworkServerRecord(raw []byte) (networkServerRecord, error) {
	var decoded networkServerRecord
	if len(raw) != binary.Size(decoded) {
		return decoded, fmt.Errorf("decode network server record: size %d, want %d", len(raw), binary.Size(decoded))
	}
	if err := binary.Read(bytes.NewReader(raw), binary.LittleEndian, &decoded); err != nil {
		return decoded, fmt.Errorf("decode network server record: %w", err)
	}
	return decoded, nil
}

func (collector *NetworkServerCollector) normalize(raw networkServerRecord) (events.Event, bool) {
	timestamp := time.Now().UTC()
	pid := int(raw.PID)
	ppid := readProcPPID(collector.procRoot, pid)
	executablePath := readProcExe(collector.procRoot, pid)
	processName := cString(raw.Comm[:])
	if executablePath != "" {
		processName = filepath.Base(executablePath)
	}
	metadata := map[string]any{
		"source":                         "ebpf_raw_tracepoint_sys_exit",
		"kernel_timestamp_ns":            raw.KernelTimestampNS,
		"completion_kernel_timestamp_ns": raw.CompletionKernelTimestampNS,
		"syscall":                        networkServerSyscallName(raw.Syscall),
		"fd":                             raw.FD,
		"return_value":                   raw.ReturnValue,
		"errno":                          syscallErrno(raw.ReturnValue),
		"outcome":                        syscallOutcome(raw.ReturnValue),
	}
	event := events.Event{
		EventID:           fmt.Sprintf("network-server-%d-%d-%d", timestamp.UnixNano(), pid, collector.sequence.Add(1)),
		Timestamp:         timestamp,
		Host:              collector.host,
		PID:               pid,
		PPID:              ppid,
		UID:               int(raw.UID),
		ProcessName:       processName,
		ParentProcessName: readProcComm(collector.procRoot, ppid),
		EventType:         events.TypeNetworkServer,
		ExecutablePath:    executablePath,
		CWD:               readProcCWD(collector.procRoot, pid),
		Metadata:          metadata,
	}
	if raw.Syscall == bindSyscallNumber {
		remoteAddr, remotePort, addressFamily, decodedMetadata, ok := decodeSockaddr(raw.Sockaddr)
		if !ok {
			return events.Event{}, false
		}
		event.RemoteAddr = remoteAddr
		event.RemotePort = remotePort
		event.Metadata["address_family"] = addressFamily
		for key, value := range decodedMetadata {
			event.Metadata[key] = value
		}
	} else {
		event.Metadata["backlog"] = raw.Backlog
	}
	enrichContainerMetadata(&event, collector.procRoot, pid, collector.containerCache)
	return event, true
}

func networkServerSyscallName(number uint32) string {
	switch number {
	case bindSyscallNumber:
		return "bind"
	case listenSyscallNumber:
		return "listen"
	default:
		return "unknown"
	}
}
